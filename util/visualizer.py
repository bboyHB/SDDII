...
class Visualizer():
    ...
    def plot_eval_results(self, epoch, results, counter_ratio=0):
        """display the current losses on visdom display: dictionary of error labels and values

        Parameters:
            epoch (int)           -- current epoch
            counter_ratio (float) -- progress (percentage) in the current epoch, between 0 to 1
            results (OrderedDict)  -- training eval results stored in the format of (name, float) pairs
        """
        ...
        try:
            self.vis.line(
                ...
                opts={
                    'title': self.name + ' eval results over time',
    ...
    def display_eval_visuals(self, visuals, epoch, save_result):
        """Display current results on visdom; save current results to an HTML file.
        ...
        if self.display_id > 0:  # show images in the browser using visdom
            ...
            if ncols > 0:        # show all the images in one visdom panel
                ...
                if label_html_row != '':
                    label_html += '<tr>%s</tr>' % label_html_row
                try:
                    self.vis.images(images, nrow=ncols, win=self.display_id + 5,
                                    padding=2, opts=dict(title=title + ' images'))
                    label_html = '<table>%s</table>' % label_html
                    self.vis.text(table_css + label_html, win=self.display_id + 6,
                                  opts=dict(title=title + ' labels'))
                except VisdomExceptionBase:
                    self.create_visdom_connections()
