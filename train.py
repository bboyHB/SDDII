...
if __name__ == '__main__':
    ...
    for epoch in range(opt.epoch_count, opt.n_epochs + opt.n_epochs_decay + 1):    # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        if opt.ifeval:
            print(epoch)
            # model.netG.eval()
            IOU, Pre, Rec, F1 = eval_when_train_p2p(opt, model.netG)
            mode = 'w' if epoch == 1 else 'a'
            with open(os.path.join(opt.checkpoints_dir, opt.name, 'iouprerecf1s.txt'), mode) as writer:
                writer.write(f"{epoch},{IOU},{Pre},{Rec},{F1}\n")
            visualizer.plot_eval_results(epoch, {'eval_IOU': IOU, 'eval_Pre': Pre, 'eval_Rec': Rec, 'eval_F1': F1})
            # model.netG.train()
        if opt.iffid:
            print(epoch)
            model.netG_A.eval()
